...
async def synthesize_segment(*, text, voice, language="Bengali",
    ...
    if gemini_keys:
        ...
        if cached:
            _log("━━━━━━━━━━━━━━━━━━")
            _log(f"[{lbl}] 💾 CACHE HIT", "success")
            _log(f"[{lbl}] API CALL SAVED", "success")
            _log("━━━━━━━━━━━━━━━━━━")
    ...
    # ── Key hit counter init ───────────────────────────────────────────
    if job is not None:
        job.setdefault("key_hits", {})

    for round_num in range(MAX_ROUNDS):
        if job and job.get("stop_requested"): raise InterruptedError("stopped")

        _log(f"[{lbl}] ROUND {round_num+1}/{MAX_ROUNDS}")
        round_ok = False
        ...
        for _ in range(len(gemini_keys)):
            ...
            # Increment hit counter and log
            job["key_hits"][str(ki+1)] = job["key_hits"].get(str(ki+1), 0) + 1
            hits = job["key_hits"][str(ki+1)]
            _log(f"[{lbl}] 🔑 {klbl} → HIT #{hits}")
        ...
        if not round_ok and round_num < MAX_ROUNDS - 1:
            ...
            for remain in range(COOLDOWN, 0, -1):
                if job and job.get("stop_requested"): raise InterruptedError("stopped")
                if remain % 5 == 0 or remain <= 10:
                    _log(f"[{lbl}] ⏱ Cooldown {remain}s remaining")
...
def _run_tts_job(job_id):
    ...
    try:
        for i, seg in enumerate(segments):
            ...
            job["current"] = i
            job_log(job, "━━━━━━━━━━━━━━━━━━")
            job_log(job, f"SEGMENT {i+1}/{job['total']}")
            job_log(job, f"TEXT: {seg['text'][:120]}")
            job_log(job, "━━━━━━━━━━━━━━━━━━")
            save_job(job)
